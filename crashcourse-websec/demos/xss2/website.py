from flask import Flask, request, session, render_template, redirect, url_for, g, make_response
...
secret_key = "asjdoajeuj9823hjflaiduhbb8943h1809qwie0aiacsodj"
def logged_in():
...
def get_database_conn():
  g.db = MySQLdb.connect(host="localhost", user="xss",
                         passwd="xss", db="xss")
...
def login():
  ...
  if request.method == 'POST':
    ...
    if hsh == p:
      ...
      if username=='mserrano':
        resp.set_cookie('super_secret', 'Congratulations on a successful XSS attack!')
      return resp
    else:
...
def messages():
  ...
  try:
    c = g.db.cursor()
    user = request.cookies.get('username')
    c.execute("SELECT id FROM messages WHERE dst=%s", user)
    rows = c.fetchall()
    c.close()
    success = True
  except:
...
def send():
  if request.method == 'POST':
    ...
    try:
      c = g.db.cursor()
      c.execute("SELECT * FROM accounts WHERE username=%s", dst)
      rows = c.fetchall()
      c.close()
      if len(rows) > 0:
        c = g.db.cursor()
        c.execute("INSERT INTO messages (src, dst, msg) VALUES (%s,%s,%s)", (username, dst, msg))
        c.close()
        g.db.commit()
        success = True
    except:
      pass
    if success:
      return render_template("sent.html", target=dst)
    else:
